...
// Move include, lib, and bin files FROM C:\Program Files\MySQL\Connector C++ 8.0
//   to the solution directory to link

// dev/include/(include files)
// dev/lib/*.lib
// ProjectDir/*.dll
...
CreateAccountFailureReason CreateAccount(const std::string& email, const std::string& plaintextPassword) {
    ...
    uint64_t userId{0};
    bool userExists = ReadUserInWebAuth(email, salt, hashedPassword, userId);
    ...
    if (result > 0) {
        uint64_t userId = ReadUserMaxIdInUser();
        if (userId == 0) {
        ...
        result = CreateUserInWebAuth(email.c_str(), salt.c_str(), hashedPassword.c_str(), std::to_string(userId));
    }
...
int main(int argc, char** argv) {
    ...
    g_MySQLDB.ConnectToDatabase("127.0.0.1:3306", "root", "root", "chat");
...
#if 0
    // Test CreateAccount
    auto reason = CreateAccount("cybercser@gmail.com", "passwd1337");
    switch (reason) {
    ...
    // Test Authenticate
    uint64_t userId{0};
    auto reason = Authenticate("cybercser@gmail.com", "passwd1337", userId);
    switch (reason) {
